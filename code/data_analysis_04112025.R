...
icb_plot <- function(data, x, y) {
  ggplot(data, aes({{x}}, {{y}}, colour = treatment)) +
    ...
    geom_signif(comparisons = list(c("control", "flupyradifurone"), c("control", "imidacloprid"), c("flupyradifurone", "imidacloprid")),
                step_increase = 0.1, 
                color = "black")+ ## easy way to draw pairwise lines. real modeled significance edited manually in Adobe Illustrator
...
nest_data$pc2[md.ind] <- pca$x[,2]

ggplot(nest_data, aes(x = pc1, y = pc2, color = treatment, shape = treatment)) + geom_point(alpha = 0.1) + scale_color_manual(values = icb_cols) + stat_ellipse()
...
ind_nest_data <- nest_data %>% group_by(bee_ID, microcolony, round, treatment, total_deployment_time, day_or_night) %>% summarise(
  ...
  act_scope = diff(quantile(meanAct, probs = c(0.15, 0.85), na.rm = TRUE)),
...
ggplot(ind_nest_data, aes(x = pc1_mean, y = pc2_mean, color = treatment)) + geom_point(alpha = 0.75)+ scale_color_manual(values = icb_cols) + stat_ellipse()

#Replace zeros and ones for activity data
ind_nest_data$act_scope[ind_nest_data$act_scope <= 0.001] <- 0.001
ind_nest_data$act_scope[ind_nest_data$act_scope >= 0.999] <- 0.999
...
p3 <- icb_plot(ind_nest_data, treatment, act_scope)+ 
  labs (y = "Activity Scope \n (85th - 15th percentile)", x = "")+ 
  facet_wrap(~day_or_night)+ 
  stat_summary(fun = "median", geom = "crossbar", fatten = 3) + 
  theme(strip.text.x = element_blank())
p3
...
p4 <- icb_plot(ind_nest_data, treatment, log10(mean_meanSpeed)) + 
  labs (y = "log10 (speed when moving)", x = "Treatment")+ 
  facet_wrap(~day_or_night)+ 
  stat_summary(fun = "median", geom = "crossbar", fatten = 3) + 
  theme(strip.text.x = element_blank())
p4
...
p5 <- icb_plot(nest_and_ft, treatment, bouts_per_24h) + 
  labs (y = "Number of daily foraging transits", x = "Treatment")+
  stat_summary(shape =15, color = "black")
p5

ft.mod <- glmer.nb(bouts_per_24h~treatment+(1|microcolony), data = nest_and_ft)
...
microcols <- live_bee %>% group_by(round,treatment,microcolony,total_deployment_time) %>% summarize(
  col_size = n(),
  total_deployment_time = unique(total_deployment_time)
) 
microcols$total_deployment_time <- ifelse(microcols$total_deployment_time < 20, mean(microcols$total_deployment_time, na.rm = TRUE), microcols$total_deployment_time)
