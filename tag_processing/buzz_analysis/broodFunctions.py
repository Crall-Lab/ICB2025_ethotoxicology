...
def medianClosestBroodDistM(broodLR):
    ...
    if brood.shape[1] > 0:   
        ...
        out = closest.median()
        out.index = [float(i.split('M')[1]) for i in out.index]
        return out
    else:
        row = broodLR.centroidX.iloc[0]
        out = pd.Series(index = row.index)
...
def PropBroodTime(broodLR):
    ...
    if brood.shape[1] > 0:   
        ...
        closest = brood.T.groupby(brood.T.index).min().T
        out = closest < onDist
        out.columns = [float(i.split('M')[1]) for i in out.columns]
        return out.mean()
    else:
        row = broodLR.centroidX.iloc[0]
        out = pd.Series(index = row.index)
...
def PropInactiveTime(broodLR):
    ...
    if work.shape[1] > 0:   
        ...
        working.columns = [float(i.split('M')[1]) for i in working.columns]
