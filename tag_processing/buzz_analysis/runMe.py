...
def processBrood(base, oneLR, name, ext, broodSource):
    ...
    ext = '-nest_image.csv'
    #print(ext)
    #broodMapPath = os.path.join(broodSource, '_'.join(base.split('_')[0:2]) + ext)
    broodMapPath = broodSource
    print(broodMapPath)
    ...
    brood = brood[brood['label'] != 'Nest perimeter (polygon)']
...
def main():
    ...
    for dir, subdir, files in os.walk(opt['source']):
        for f in files:
            ...
            trackingResults = trackingResults.loc[trackingResults['frame']<80]
